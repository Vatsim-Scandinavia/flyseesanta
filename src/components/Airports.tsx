...
const convertMpsToKnots = (mps: number) =>
  (mps * 1.94384).toFixed(0) + " kts";

// Fetch weather data
const fetchWeather = async ({ fmisid, place }: { fmisid?: number; place?: string }) => {
  let url = "";
  ...
  try {
    ...
    // First valid row is usually the latest observation
              ...
              <div className="p-6 pt-4 flex flex-col gap-4 flex-grow">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.details.map((t) => (
                    <span key={t} className="px-3 py-1 rounded text-xs font-medium bg-white/5 border border-white/20 text-gray-200">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-auto flex flex-wrap items-center gap-2">
                  {project.charts && (
                    <a href={project.charts} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 px-3 py-1 rounded bg-blue-800 border border-white/10 text-xs font-semibold hover:bg-blue-900 transition">
